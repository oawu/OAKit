...
public extension OA.Timer {
    private static var _all: [String: Foundation.Timer] = [:]

    @discardableResult static func has(key: String) -> Bool { self._all[key] != nil }

    @discardableResult static func clean(key: String) -> Bool {
        if let tmp = self._all[key] { tmp.invalidate() }
        self._all.removeValue(forKey: key)
        return self._all[key] == nil
    }

    @discardableResult static func cleanAll() -> Bool {
        self._all.forEach { key, _ in self.clean(key: key) }
        return true
    }

    @discardableResult static func delay(key: String, second: TimeInterval, replace: Bool = true, repeats: Bool = false, block: @escaping () -> ()) -> Foundation.Timer {
    ...
    @discardableResult static func loop(key: String, second: TimeInterval, replace: Bool = true, delay: Bool = false, block: @escaping () -> ()) -> Foundation.Timer {
        if let timer = Self._all[key], replace {
            timer.invalidate()
            guard Self.clean(key: key) else { return timer }
        }
